...
class ProfileController extends Controller
{
    ...
    public function edit()
    {
        ...
        // Only HR needs these lookups:
        $departments     = Department::orderBy('name')->pluck('name','id');
        $designations    = Designation::orderBy('name')->pluck('name','id');
        $schedules       = Schedule::orderBy('name')->pluck('name','id');
        $employmentTypes = [
            'regular'      => 'Regular',
            'casual'       => 'Casual',
            'project'      => 'Project',
    ...
    public function update(Request $request)
    {
        ...
        if (! $isHr && $employee->profile_updated_at) {
            $daysSince = Carbon::parse($employee->profile_updated_at)
                              ->diffInDays(now());
            if ($daysSince < 30) {
                $wait = 30 - $daysSince;
                return back()
                    ->withErrors(['too_soon' => "Please wait {$wait} more day(s)."])
                    ->withInput();
            }
        }
        ...
        if ($isHr) {
            $rules = array_merge($rules, [
                ...
                'fingerprint_id'       => ['nullable','string', Rule::unique('employees')->ignore($employee->id)],
            ]);
        }
        ...
        $fill = [
            ...
            'profile_picture'  => $data['profile_picture'] ?? $employee->profile_picture,
        ];
