...
use Illuminate\Support\Facades\Storage;        // ⬅️ needed by face image save
...
use App\Models\FaceTemplate;                  // ⬅️ face template model
...
class EmployeeController extends Controller
{
    ...
    {
        // ⬅️ FIX: pass simple id => name arrays so selects don't show JSON
        $departments     = Department::orderBy('name')->pluck('name','id')->toArray();
        $designations    = Designation::orderBy('name')->pluck('name','id')->toArray();
        $schedules       = Schedule::orderBy('name')->pluck('name','id')->toArray();
        $roles           = Role::pluck('name','name')->toArray();
    ...
    public function store(Request $request)
    {
        $data = $request->validate([
            ...
            'face_image_base64' => 'nullable|string',  // data:image/png;base64,...
        ]);

        DB::beginTransaction();

        try {
            // 1) Create the User (pending)
            ...
            // 4) OPTIONAL: Save face template if provided
            if (!empty($data['face_descriptor'])) {
                try {
                    $desc = json_decode($data['face_descriptor'], true, 512, JSON_THROW_ON_ERROR);
                    if (!is_array($desc) || count($desc) !== 128) {
                        throw new \RuntimeException('Face descriptor must be an array of 128 floats.');
                    }
                    ...
                    Log::warning('Face template not saved: '.$fe->getMessage());
                    session()->flash('warning', 'Employee saved, but face template was not stored: '.$fe->getMessage());
                }
            }
